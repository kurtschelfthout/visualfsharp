...
let UnifyTypes cenv (env: TcEnv) m expectedTy actualTy = 
    ConstraintSolver.AddCxTypeEqualsType (MakeWitnessEnv cenv.g env.NameEnv) env.eContextInfo env.DisplayEnv cenv.css m (tryNormalizeMeasureInType cenv.g expectedTy) (tryNormalizeMeasureInType cenv.g actualTy)
...
let UnifyRefTupleType contextInfo cenv wenv denv m ty ps = 
    let ptys = 
...
/// Optimized unification routine that avoids creating new inference 
/// variables unnecessarily
let UnifyStructTupleType contextInfo cenv wenv denv m ty ps = 
    let ptys = 
...
let UnifyUnitType cenv wenv denv m ty exprOpt =
    if AddCxTypeEqualsTypeUndoIfFailed wenv denv cenv.css m ty cenv.g.unit_ty then 
        true
    else
        let domainTy = NewInferenceType ()
        let resultTy = NewInferenceType ()
...
module GeneralizationHelpers = 
    ...
    let CanonicalizePartialInferenceProblem (cenv,wenv,denv,m) tps =
        ...
        let csenv = (MakeConstraintSolverEnv ContextInfo.NoContext cenv.css m wenv denv)
    ...
    let (tpsorig,_, _, _, tinst, _) as res = 
        ...
        if isByrefTy cenv.g vty then 
          ...
          | Some c -> 
              ...
              //   [<Literal>]
              //   let Null = null
              let tpsorig,_,tinst,tau = FreshenPossibleForallTy cenv.g (MakeWitnessEnv cenv.g env.NameEnv) m TyparRigidity.Flexible vty 
...
let LightweightTcValForUsingInBuildMethodCall g wenv (vref:ValRef) vrefFlags (vrefTypeInst : TTypes) m = 
    ...
    if isByrefTy g vty then 
      ...
      match v.LiteralValue with 
      | Some c -> 
          let _,_,_,tau = FreshenPossibleForallTy g wenv m TyparRigidity.Flexible vty 
          Expr.Const(c,m,tau),tau
      | None -> 
              // Instantiate the value 
              let tau = 
                  // If we have got an explicit instantiation then use that 
                  let _,tps,tptys,tau = FreshenPossibleForallTy g wenv m TyparRigidity.Flexible vty 
...
let TcRuntimeTypeTest isCast isOperator cenv wenv denv m tgty srcTy =
...
let rec TcTyparConstraint ridx cenv newOk checkCxs occ (env: TcEnv) tpenv c = 
    ...
    | WhereTyparDefaultsToType(tp,ty,m) ->
        ...
        let tp',tpenv = TcTypar cenv env newOk tpenv tp
        let csenv = MakeConstraintSolverEnv env.eContextInfo cenv.css m (MakeWitnessEnv cenv.g env.NameEnv) env.DisplayEnv
...
and TcPat warnOnUpper cenv env topValInfo vFlags (tpenv,names,takenNames) ty pat = 
    ...
    | SynPat.Named (SynPat.IsInst(cty,m),_,_,_,_) -> 
        ...
        TcRuntimeTypeTest (*isCast*)false (*isOperator*)true cenv (MakeWitnessEnv cenv.g env.NameEnv) env.DisplayEnv m tgty srcTy
        match pat with 
        | SynPat.IsInst(_,m) ->
...
and solveTypAsError cenv wenv denv m ty =
    ...
    SolveTypEqualsTypKeepAbbrevs (MakeConstraintSolverEnv ContextInfo.NoContext cenv.css m wenv denv) 0 m NoTrace ty ty2 |> ignore
...
and TcExprUndelayed cenv overallTy env tpenv (expr: SynExpr) =
    ...
    | SynExpr.Downcast(e,_,m) | SynExpr.InferredDowncast (e,m) ->
        ...
        let tgty,tpenv,isOperator = 
          ...
          | _ -> failwith "downcast"
        TcRuntimeTypeTest (*isCast*)true isOperator cenv (MakeWitnessEnv cenv.g env.NameEnv) env.DisplayEnv m tgty srcTy
    ...
    | SynExpr.Tuple (args,_,m) -> 
        let argtys = UnifyRefTupleType env.eContextInfo cenv (MakeWitnessEnv cenv.g env.NameEnv) env.DisplayEnv m overallTy args
    ...
    | SynExpr.StructTuple (args,_,m) -> 
        let argtys = UnifyStructTupleType env.eContextInfo cenv (MakeWitnessEnv cenv.g env.NameEnv) env.DisplayEnv m overallTy args
        // No subsumption at tuple construction
        let flexes = argtys |> List.map (fun _ -> false)
        let args',tpenv = TcExprs cenv env m tpenv flexes argtys args
...
and TcMethodApplication 
    ...
    let uniquelyResolved,preArgumentTypeCheckingCalledMethGroup = 
        ...
        let uniquelyResolved = 
            let csenv = MakeConstraintSolverEnv ContextInfo.NoContext cenv.css mMethExpr wenv denv
...
and TcNormalizedBinding declKind (cenv:cenv) env tpenv overallTy safeThisValOpt safeInitInfo (enclosingDeclaredTypars,(ExplicitTyparInfo(_,declaredTypars,_) as flex)) bind =
    ...
    | NormalizedBinding(vis,bkind,isInline,isMutable,attrs,doc,_,valSynData,pat,NormalizedBindingRhs(spatsL,rtyOpt,rhsExpr),mBinding,spBind) ->
        ...
        if bkind = StandaloneExpression && not cenv.isScript then 
            UnifyUnitType cenv (MakeWitnessEnv cenv.g env.NameEnv) env.DisplayEnv mBinding overallPatTy (Some rhsExprChecked) |> ignore<bool>
                    ...
                    tp.Constraints |> List.iter (fun tpc -> 
                        ...
                        | TyparConstraint.DefaultsTo(priority2,ty2,m) when priority2 = priority -> 
                            ...
                            if not tp.IsSolved  && not (typeEquiv cenv.g ty1 ty2) then
                                let csenv = MakeConstraintSolverEnv ContextInfo.NoContext cenv.css m wenv denvAtEnd
